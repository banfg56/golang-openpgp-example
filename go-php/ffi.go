...
func _toJson( _data map[string]interface{} )(*C.char){
	res1B, err := json.Marshal(_data)
	if err != nil {
		return C.CString("")
	}
	// fmt.Println("return\n", string(res1B))
	return C.CString( string(res1B) )
}

//export enCryptSign
func enCryptSign(_encryptionText *C.char, _pubKeyringFile *C.char, _privKeyringFile *C.char, _passPhrase *C.char) (*C.char) {
	sRet :=  map[string]interface{} {"code":500, "msg":"System Error", "data": nil}
	
	encryptionText := C.GoString(_encryptionText)
	pubKeyringFile := C.GoString(_pubKeyringFile)
	privKeyringFile := C.GoString(_privKeyringFile)
	passPhrase	    := C.GoString(_passPhrase)
	// Read in public key
...
//export deCrypt
func deCrypt(_encStrs *C.char, _pubKeyringFile *C.char, _privKeyringFile *C.char, _passPhrase *C.char) (*C.char) {
	sRet :=  map[string]interface{} {"code":500, "msg":"System Error", "data": nil}

	encStrs := C.GoString(_encStrs)
	pubKeyringFile := C.GoString(_pubKeyringFile)
	privKeyringFile := C.GoString(_privKeyringFile)
	passPhrase := C.GoString(_passPhrase)
